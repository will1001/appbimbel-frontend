...
function Others(props) {
    ...
    const selectDataChanged = (e) => {
        setSelectData(e.target.value);
        switch(e.target.value) {
            // case "bab_soal":
            //   setTh(["no", "judul","Kategori", "Deskripsi", "Action"]);
            //   break;
            case "sub_bab_soal":
                setTh(["no","Deskripsi","Bab", "Action"]);
                setTd(subBabSoal.map((item,index) => (
                    <tr key={item.id}>
                        <td>{index+1}</td>
                        <td>{item.deskripsi}</td>
                        <td>{item.id_bab}</td>
                        <td><a href="#0" onClick={()=>editData(item)}>edit</a> || <a href="#0" onClick={()=>hapusQuiz(item.id)}>hapus</a></td>
                    </tr>
                    )));
              break;
            case "konsep_soal":
                setTh(["no","Deskripsi","Sub Bab", "Action"]);
                setTd(konsepSoal.map((item,index) => (
                    <tr key={item.id}>
                        <td>{index+1}</td>
                        <td>{item.deskripsi}</td>
                        <td>{item.id_sub_bab}</td>
                        <td><a href="#0" onClick={()=>editData(item)}>edit</a> || <a href="#0" onClick={()=>hapusQuiz(item.id)}>hapus</a></td>
                    </tr>
                    )));
              break;
            // case "level_soal":
            //     setTh(["no","Deskripsi", "Action"]);
            //     setTd(tingkatKesulitan.map((item,index) => (
            //         <tr key={item.id}>
            //             <td>{index+1}</td>
            //             <td>{item.deskripsi}</td>
    ...
    }, [refreshRender,selectMapel,selectbabSoal,kategoriSoal]);
...
/// action function
    ...
    const editData = (data) => {
       ...
       setId(data.id);
    //    if(selectData === "sub_bab_soal"){
           setSelectBabSoal(data.id_bab);
    //    }
    //    if(selectData === "konsep_soal"){
           setSelectSubBabSoal(data.id_sub_bab);
    //    }
    ...
    const mainPanel =
        <>
            ...
            <span>Tabel Data</span>
                    <select value={selectData} onChange={selectDataChanged}>   
                    <option value="" disabled ></option>
                        {listData.map(item => (
